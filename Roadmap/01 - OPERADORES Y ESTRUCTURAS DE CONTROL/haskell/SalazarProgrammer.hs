...
{-|
  ...
  Bits: .&., .|., xor, complement
   ...
   - Condicionales: if-then-else, guardas (|), pattern matching
   - Iterativas: Recursión, map, filter, foldl/foldr
   Excepciones: Maybe, Either (en lugar de try-catch)

   Diferencia clave:
    Haskell es funcional puro, por lo que:
    - No hay operadores de asignación (= es para definición, no asignación)
    - No hay bucles tradicionales (se usa recursión y funciones de orden superior)
    - El manejo de errores se hace con tipos como Maybe y Either

   Dificultad Extra:
    La solución usa una lista por comprensión que filtra números:
    - Entre 10 y 55 ([10..55])
    - Pares (even x)
    - Diferentes de 16 (x /= 16)
    - No múltiplos de 3 (x mod 3 /= 0)
-}
