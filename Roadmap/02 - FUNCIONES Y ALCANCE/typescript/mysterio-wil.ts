...
function greetDefault(name: string, greeting: string = "Hola"): void {
...
printArgs(1, "texto", true, { framework: "TypeScript" });
...
console.log(`Usando la propiedad 'length' de un array: El array tiene ${myList.length} elementos.`);
console.log(`Usando Math.max(): El valor máximo del array es ${Math.max(...myList)}`);


console.log("\n===> Variable LOCAL y GLOBAL <===");
let globalVar: string = "Soy una variable global";

function myFunctionScope(): void {
    const localVar: string = "Soy una variable local";
 ...
 */
console.log("\n====> DIFICULTAD EXTRA <====");
function fizzBuzzExtra(text1: string, text2: string): number {
    let count = 0;
    for (let i = 1; i <= 100; i++) {
...
const timesPrintedNumber: number = fizzBuzzExtra("Fizz", "Buzz");
